...
function UserContent() {
    ...
    const [outings, setOutings] = useState<OutingItem[]>([]);
    ...
    useEffect(() => {
        ...
        groupsService.getGroupMembers(gId).then((result) => {
            if (result.isOk) {
                const found = result.data.members.find((m) => m.id === userId) ?? null;
                setMember(found);
            }
        });
        ...
        groupsService.getGroupOutings(gId).then((result) => {
            if (result.isOk) {
                setOutings(result.data.outings.filter((o) => o.isParticipant));
            }
        });
    }, [userId]);
            ...
            <main className="flex w-full max-w-md min-h-screen flex-col items-start p-6 gap-6">
                ...
                <div className="w-full flex items-center">
                    ...
                    <p className="flex-1 text-center text-[28px] font-['Nunito_Sans'] font-extrabold text-[#001A0E] leading-[36px] tracking-[-0.7px] dark:text-zinc-50">
                        {member ? `Profil de ${member.firstName}` : "Profil"}
                    </p>
                </div>
                            ...
                            {member.interests && member.interests.length > 0 && (
                                <div className="flex flex-wrap justify-center gap-2 mt-1">
                                    {member.interests.map((item, i) => (
                                        <span key={i} className="px-3 py-1 rounded-full bg-[#EEF3FB] dark:bg-zinc-800 text-sm text-zinc-700 dark:text-zinc-200">
                                            {item}
                                        </span>
                                    ))}
                                </div>
                ...
                <div className="w-full flex flex-col gap-3 mt-4">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Sorties en commun</p>
                    </div>

                    {outings.length === 0 ? (
                        <p className="text-sm text-zinc-400">Aucune sortie en commun.</p>
                    ) : (
                        <ul className="flex flex-col gap-2 w-full">
                            {outings.map((o) => (
                                <li key={o.id} className="flex justify-between items-center rounded-xl bg-zinc-100 dark:bg-zinc-800 px-4 py-3">
